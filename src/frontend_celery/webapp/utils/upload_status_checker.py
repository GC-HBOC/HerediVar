...
def check_update_clinvar_status(variant_id, conn: Connection, force_update = False):
    clinvar_queue_entry = conn.get_most_recent_publish_clinvar_queue_entry(variant_id) # id, publish_queue_id, requested_at, status, message, submission_id, accession_id, last_updated, celery_task_id, consensus_classification_id
    ...
    if (status in ['processing', 'submitted', 'retry', 'process', 'pending'] and submission_id is not None) or force_update:
        ...
        clinvar_queue_entry = conn.get_most_recent_publish_clinvar_queue_entry(variant_id)
