...
function create_exclamation_mark() {
    var image = document.createElementNS("http://www.w3.org/2000/svg", "svg");
...
function show_status(color_class, tooltip_text, inner_text, pill_holder_id, pill_id, prepend_html=null) {
    ...
    if (prepend_html != null) {
        status_pill.classList.add("d-flex")
        status_pill.classList.add("align-items-center")
        prepend_html.classList.add("ssr")
        status_pill.appendChild(prepend_html)
    }
    status_pill.appendChild(get_div(inner_text, []))
    pill_holder.appendChild(status_pill) // add new pill
}


function get_div(text_content, classes, default_text = "") {
    var res = document.createElement("div")

    classes.forEach(el => {
        res.classList.add(el)
    });

    if (text_content == null) {
        res.textContent = default_text
    } else {
        res.textContent = text_content
    }
